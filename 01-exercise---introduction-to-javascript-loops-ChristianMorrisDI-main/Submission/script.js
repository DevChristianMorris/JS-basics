...
for (let index = 0; index < couponsPartB.length; index++) {
  const couponB = couponsPartB[index];
  couponB.expired = couponB.expiresAt > new Date().getTime();
}
// Part C - For...of loop
const couponsPartC = [
  {
...
for (const couponC of couponsPartC) {
  couponC.expired = couponC.expiresAt > new Date().getTime();
}